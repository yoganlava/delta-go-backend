...
run: ## run the API server
	go run ${LDFLAGS} internal/main.go
...
run-restart: ## restart the API server
	...
	@go run ${LDFLAGS} internal/main.go & echo $$! > $(PID_FILE)
	@printf '%*s\n' "80" '' | tr ' ' -

run-live: ## run the API server with live reload support (requires fswatch)
	@go run ${LDFLAGS} internal/main.go & echo $$! > $(PID_FILE)
	@fswatch -x -o --event Created --event Updated --event Renamed -r internal db cmd config | xargs -n1 -I {} make run-restart
