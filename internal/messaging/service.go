...
func (ms MessagingService) SendMessage(message dto.CreateMessageDTO) error {
	_, err := ms.pool.Exec(context.Background(), `
	insert into
	messages
	(sender_id, receiver_id, subject, body, created_at)
	values
	($1, $2, $3, $4, now())`,
		message.SenderID, message.ReceiverID, message.Subject, message.Body,
	)
	return err
}
...
func (ms MessagingService) RetrieveUserMessages(userID int) ([]*entity.Message, error) {
	...
	err := pgxscan.Select(context.Background(), ms.pool, &m, `
	select
	sender_id, receiver_id, subject, body, created_at
	from message
	where receiver_id = $1`,
